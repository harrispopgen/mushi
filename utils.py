...
def C(n: int) -> np.ndarray:
    ...
    '''
    W1 = np.zeros((n - 1, n - 1), dtype=np.longdouble)
    W2 = np.zeros((n - 1, n - 1), dtype=np.longdouble)
    b = np.arange(1, n - 1 + 1, dtype=np.longdouble)
    # j = 2
    W1[:, 0] = 6 / (n + 1)
    W2[:, 0] = 0
    # j = 3
...
def M(n: int, η: histories.η) -> np.ndarray:
    ...
    return np.diag(1 / binom_vec) \
        @ np.cumprod(u[np.newaxis, :-1],
                     axis=1, dtype=np.longdouble) ** binom_vec[:, np.newaxis] \
