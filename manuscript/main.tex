...
\begin{align}
\ell(\boldsymbol y, \boldsymbol z; \boldsymbol x) &= \sum_{b=1}^{n-1}\left(x_b\log(\xi_b) - \xi_b - \log(x_b!)\right)\\
...
\subsection{Regularization}\label{sec:model:reg}
...
We define weighted $L_p^p$ derivative regularizers for arbitrary histories $\eta(t)$ and $\mu(t)$:
\[
\int_0^\infty\left|\frac{d\eta(t)}{dt}\right|^p dt, \int_0^\infty\left|\frac{d\mu(t)}{dt}\right|^p dt.
\]
With our piecewise constant history $(\boldsymbol y, \boldsymbol z)$ the derivatives become Dirac delta functions at the change points, and these become
\[
\left|\Delta\boldsymbol y\right|_p^p, \left|\Delta\boldsymbol z\right|_p^p.
\]
where $\Delta$ denotes the first difference operator.

Our fused LASSO \citep{tibs} loss function is then
\begin{align}
L(\boldsymbol y, \boldsymbol z) &= - \ell(\boldsymbol y, \boldsymbol z) + \lambda_\eta \left(\alpha\left|\Delta\boldsymbol y\right|_1 + \frac{(1-\alpha)}{2}\left|\Delta\boldsymbol y\right|_2^2\right) + \lambda_\mu \left(\alpha\left|\Delta\boldsymbol z\right|_1 + \frac{(1-\alpha)}{2}\left|\Delta\boldsymbol z\right|_2^2\right),
\end{align}
