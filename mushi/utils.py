...
def tmrca_sf(t: np.ndarray, y: np.ndarray, n: int) -> np.ndarray:
    ...
    result = result.at[:-1].set(
        np.squeeze(
            A2[np.newaxis, :]
            @ np.exp(np.cumsum(logu[np.newaxis, :-1], axis=1)) ** binom_vec
        )
    )
...
def misid_partners(mutation_types: List[str]) -> List[int]:
    """Ancestral state misidentification partner indices. Mutation type
    ``None`` is self-partnered.
    ...
    while to_pair:
        ...
        if mutype1 is None:
            ...
            if not match:
                raise ValueError(
                    "no ancestral misidentification partner found "
